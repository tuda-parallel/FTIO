    ...
    for attempt in range(_MAX_RETRIES):
        ...
        if not timed_out:
            ...
            if (
                ...
                if settings.fuse:
                    start_fuse(settings)
                # Lower the FTIO gate so pre-app I/O during re-staging is not
                # fed into the predictor (app_start_file was already created
                # before start_application was first called).
                _flag_removed = False
                if (
                    not settings.exclude_ftio
                    and settings.app_start_file
                    and os.path.isfile(settings.app_start_file)
                ):
                    os.remove(settings.app_start_file)
                    _flag_removed = True
                ...
                pre_call(settings)
                # Restore the gate so FTIO resumes tracking application I/O.
                if _flag_removed:
