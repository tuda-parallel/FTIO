...
def main(argv=sys.argv):
    ...
    combined_df_long = pd.melt(combined_df, id_vars=["Source File"], var_name="Metric", value_name="Value")
...
def single_plot(df):
    ...
    fig = px.box(df, 
                ...
                labels={"Metric": "Metric", "Value": "Values"})  # Labels for the axes
...
def sub_plots(df,file_basenames):    
    ...
    num_dfs = len(file_basenames)

    # Create subplots with each row representing a metric
    fig = sp.make_subplots(rows=num_metrics, cols=1, 
    ...
    for i, metric in enumerate(metrics):
        ...
        box_trace = go.Box(
            x=metric_df["Source File"],  # x-axis as Source File (color coding)
            y=metric_df["Value"],         # y-axis as the values
            name=metric
        )
    ...
    fig.update_layout(
        ...
        width=800,                # Adjust width for visibility
