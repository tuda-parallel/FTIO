...
def heatmap(data):
    ...
    heatmap_data['Dominant Frequency Binned'] = pd.cut(heatmap_data['Dominant Frequency'], bins=freq_bins, include_lowest=True)
    ...
    for i, metric_i in enumerate(metrics_unique):
        for j, metric_j in enumerate(metrics_unique):
            if i < j:  # Only calculate for one half of the matrix
                freq_i = dominant_freq_per_metric.loc[dominant_freq_per_metric['Metric'] == metric_i, 'Dominant Frequency'].values
                freq_j = dominant_freq_per_metric.loc[dominant_freq_per_metric['Metric'] == metric_j, 'Dominant Frequency'].values
