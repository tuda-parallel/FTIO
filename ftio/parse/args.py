...
""",
    ...
    parser.add_argument(
        ...
        dest="mode",
        type=str,
        help="if the trace file contains several I/O modes, a specific mode can be selected. Supported modes are: write_async, read_async, write_sync, read_sync",
    )
    parser.add_argument(
        "-s",
        "--source",
        dest="source",
        type=str,
        help="the source of the files: tmio, or custom. See https://github.com/tuda-parallel/FTIO/blob/main/docs/file_formats.md",
    )
    ...
    if "ftio" in name.lower() or "predictor" in name.lower():
        ...
        parser.add_argument(
            ...
            dest="level",
            type=int,
            help="specifies the decomposition level for the discrete wavelet transformation (default=3). If specified as auto, the maximum decomposition level is automatic calculated",
        )
