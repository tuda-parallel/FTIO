...
def det_imf_frq(imf, center_frq, fs, args):
    ...
    if np.abs(yf[ind - 1]) > np.abs(yf[ind - 2]) and np.abs(yf[ind - 1]) > np.abs(
        yf[ind + 1]
    ):
    ...
    corr = correlation(imf, amp, method="spearman")
    ...
    for ind in indices:
        ...
        corr = correlation(amp_env, amp_cos, method="spearman")
...
def imf_select_windowed(signal, t, u_per, fs, overlap=0.5):
    ...
    for j in range(0, u_per.shape[0]):
        ...
        while ind + win_size < len(imf):
            ...
            corr = correlation(
                signal[ind : ind + win_size].astype(float),
                imf[ind : ind + win_size],
                method="pearson",
            )
            if corr > 0.65 and not flag:
                ...
                if (
                    per_segments
                    and j == per_segments[-1].index
                    and start < per_segments[-1].end
                ):
...
def imf_selection(signal, u_per, t, center_freqs, fs, args):  # , u_per):
    ...
    for j in range(0, u_per.shape[0]):
        ...
        corr = correlation(signal.astype(float), imf, method="pearson")
