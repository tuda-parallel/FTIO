...
from ftio.plot.plot_wavelet_cont import (
    plot_wave_cont_and_spectrum,
    plot_scales,
    plot_scales_all_in_one,
)  # plot_spectrum, plot_wave_cont
...
def ftio_wavelet_cont(
    args: Namespace, bandwidth: np.ndarray, time_stamps: np.ndarray, ranks: int = 0
):
    """
    ...
    if any(x in args.engine for x in ["mat", "plot"]):
        ...
        f = [
            plot_wave_cont_and_spectrum(
                args, t_sampled, frequencies, power_spectrum, power_at_scale, dominant_scale, peaks
            ),
            plot_scales(args, t_sampled, b_sampled, power_spectrum, frequencies, scales),
            plot_scales_all_in_one(
                args, t_sampled, b_sampled, power_spectrum / np.max(b_sampled), frequencies, scales
            ),
        ]
        if "ploty" in args.engine:
            f.append(
                plot_scales_all_in_one(
                    args, t_sampled, b_sampled, coefficients, frequencies, scales, all_peaks
                )
            )
