...
class ChangePointDetector:
    ...
    def _get_last_change_time(self):
        if self.shared_resources:
            return (
                self.shared_resources.detector_last_change_time.value
                if self.shared_resources.detector_last_change_time.value > 0
                else None
            )
    ...
    def add_prediction(
        self, prediction: Prediction, timestamp: float
    ...
    def _add_prediction_synchronized(
        self, prediction: Prediction, timestamp: float, freq: float
    ) -> tuple[int, float] | None:
        frequencies = self._get_frequencies()
        timestamps = self._get_timestamps()
    ...
    def _test_cut_point(self, cut: int) -> bool:
        ...
        if self.verbose:
            ...
            self.console.print(
                f"  [dim]• Test: {mean_diff:.3f} > {threshold:.3f} ? {'CHANGE!' if mean_diff > threshold else 'No change'}[/]"
            )
    ...
    def _process_change_point(self, change_point: int):
        ...
        time_span = (
            new_timestamps[-1] - new_timestamps[0] if len(new_timestamps) > 1 else 0
        )

        self.console.print(
            f"[green][ADWIN] Window adapted: "
            f"{old_window_size} → {new_window_size} samples[/]"
        )
        self.console.print(
            f"[green][ADWIN] Frequency shift: "
    ...
    def get_adaptive_start_time(self, current_prediction: Prediction) -> float:
        ...
        if last_change_time is not None:
            ...
            if window_span < min_window:
                ...
                self.console.print(
                    f"[yellow][ADWIN] Change point too recent, using min window: "
                    f"{adaptive_start:.6f}s[/]"
                )
            elif window_span > max_lookback:
                adaptive_start = max(0, current_prediction.t_end - max_lookback)
                self.console.print(
                    f"[yellow][ADWIN] Change point too old, using max lookback: "
                    f"{adaptive_start:.6f}s[/]"
                )
            else:
                adaptive_start = exact_change_start
                self.console.print(
                    f"[green][ADWIN] Using EXACT change point timestamp: "
    ...
    def get_change_point_time(self, shared_resources=None) -> float | None:
...
class CUSUMDetector:
    ...
    def _update_adaptive_parameters(self, freq: float):
        ...
        if self.shared_resources and hasattr(
            self.shared_resources, "detector_frequencies"
        ):
            if hasattr(self.shared_resources, "detector_lock"):
                with self.shared_resources.detector_lock:
                    all_freqs = list(self.shared_resources.detector_frequencies)
                    recent_freqs = (
                        all_freqs[-self.window_size - 1 : -1]
                        if len(all_freqs) > 1
                        else []
                    )
            else:
                all_freqs = list(self.shared_resources.detector_frequencies)
                recent_freqs = (
                    all_freqs[-self.window_size - 1 : -1] if len(all_freqs) > 1 else []
                )
        else:
            ...
            recent_freqs = (
                self.frequency_buffer[:-1] if len(self.frequency_buffer) > 1 else []
            )

        if self.verbose:
            self.console.print(
                f"[dim magenta][CUSUM DEBUG] Buffer for σ calculation (excluding current): {[f'{f:.3f}' for f in recent_freqs]} (len={len(recent_freqs)})[/]"
            )
    ...
    def add_frequency(
        ...
        if not self.initialized:
            ...
            if len(freq_list) >= min_init_samples:
                ...
                if self.show_init:
                    self.console.print(
                        f"[yellow][AV-CUSUM] Reference established: {self.reference:.3f} Hz "
                        f"(from first {min_init_samples} observations: {[f'{f:.3f}' for f in first_freqs]})[/]"
                    )
            else:
                current_count = len(freq_list)
                self.console.print(
                    f"[dim yellow][AV-CUSUM] Collecting calibration data ({current_count}/{min_init_samples})[/]"
                )
                return False, {}
        ...
        if self.verbose:
            current_window_size = (
                len(self.shared_resources.detector_frequencies)
                if self.shared_resources
                else 0
            )
            ...
            self.console.print(
                f"  [dim]• Deviation: {freq:.3f} - {self.reference:.3f} = {deviation:.3f}[/]"
            )
            self.console.print(
            ...
            self.console.print(
                f"  [dim]• Sum_pos calculation: max(0, {self.sum_pos:.3f} + {deviation:.3f} - {self.adaptive_drift:.3f}) = {new_sum_pos:.3f}[/]"
            )
            self.console.print(
                f"  [dim]• Sum_neg calculation: max(0, {self.sum_neg:.3f} - {deviation:.3f} - {self.adaptive_drift:.3f}) = {new_sum_neg:.3f}[/]"
            )
            self.console.print(
                f"  [dim]• Adaptive threshold: {self.adaptive_threshold:.3f} (h_t = 2.0×σ, σ={self.rolling_std:.3f})[/]"
            )
            self.console.print(
                f"  [dim]• Upward change test: {self.sum_pos:.3f} > {self.adaptive_threshold:.3f} = {'UPWARD CHANGE!' if self.sum_pos > self.adaptive_threshold else 'No change'}[/]"
            )
            self.console.print(
                f"  [dim]• Downward change test: {self.sum_neg:.3f} > {self.adaptive_threshold:.3f} = {'DOWNWARD CHANGE!' if self.sum_neg > self.adaptive_threshold else 'No change'}[/]"
            )
        ...
        change_info = {
            ...
            "deviation": deviation,
            "change_type": (
                "increase" if upward_change else "decrease" if downward_change else "none"
            ),
        }
        ...
        if change_detected:
            ...
            change_percent = (
                abs(deviation / self.reference * 100) if self.reference != 0 else 0
            )

            self.console.print(
                f"[bold yellow][AV-CUSUM] CHANGE DETECTED! "
                f"{self.reference:.3f}Hz → {freq:.3f}Hz "
                f"({change_percent:.1f}% {change_type})[/]"
            )
            self.console.print(
                f"[yellow][AV-CUSUM] Sum_pos={self.sum_pos:.2f}, Sum_neg={self.sum_neg:.2f}, "
                f"Adaptive_Threshold={self.adaptive_threshold:.2f}[/]"
            )
            self.console.print(
                f"[dim yellow]AV-CUSUM ANALYSIS: Cumulative sum exceeded adaptive threshold {self.adaptive_threshold:.2f}[/]"
            )
            self.console.print(
                f"[dim yellow]Detection method: {'Positive sum (upward trend)' if upward_change else 'Negative sum (downward trend)'}[/]"
            )
            self.console.print(
            ...
            self.reference = freq
            self.console.print(
                f"[cyan][CUSUM] Reference updated: {old_reference:.3f} → {self.reference:.3f} Hz "
            ...
            if self.shared_resources:
                if hasattr(self.shared_resources, "detector_lock"):
                    with self.shared_resources.detector_lock:
                        ...
                        self.shared_resources.detector_timestamps[:] = (
                            current_timestamp_list
                        )

                        self.console.print(
                            f"[green][CUSUM] CHANGE POINT ADAPTATION: Discarded {old_window_size-1} past samples, "
                            f"starting fresh from current detection[/]"
                        )
                        self.console.print(
                            f"[green][CUSUM] WINDOW RESET: {old_window_size} → {len(self.shared_resources.detector_frequencies)} samples[/]"
                        )
...
class SelfTuningPageHinkleyDetector:
    ...
    def _reset_detector_state(self):
        ...
        self.console.print(
            "[dim yellow][STPH] State cleared: Starting fresh when frequency resumes[/]"
        )
    ...
    def reset(self, current_freq: float = None):
            ...
            if hasattr(self.shared_resources, "detector_lock"):
                with self.shared_resources.detector_lock:
                    ...
                    if hasattr(self.shared_resources, "detector_timestamps"):
                        if current_freq is not None:
                            last_timestamp = (
                                self.shared_resources.detector_timestamps[-1]
                                if len(self.shared_resources.detector_timestamps) > 0
                                else 0.0
                            )
                            self.shared_resources.detector_timestamps[:] = [
                                last_timestamp
        ...
        if current_freq is not None:
            self.console.print(
                f"[cyan][PH] Internal state reset with new reference: {current_freq:.3f} Hz[/]"
            )
        else:
            self.console.print(
                "[cyan][PH] Internal state reset: Page-Hinkley parameters reinitialized[/]"
            )
    ...
    def add_frequency(
        ...
        if self.verbose:
            self.console.print(
                f"[dim magenta][STPH DEBUG] Sample #{self.sample_count}:[/]"
            )
            self.console.print(f"  [dim]• Current freq: {freq:.3f} Hz[/]")
            self.console.print(
            ...
            self.console.print(
                f"  [dim]• Positive difference: {freq:.3f} - {self.reference_mean:.3f} - {self.adaptive_delta:.3f} = {pos_difference:.3f}[/]"
            )
            self.console.print(
                f"  [dim]• Sum_pos = max(0, {old_cumsum_pos:.3f} + {pos_difference:.3f}) = {self.cumulative_sum_pos:.3f}[/]"
            )
            self.console.print(
                f"  [dim]• Negative difference: {self.reference_mean:.3f} - {freq:.3f} - {self.adaptive_delta:.3f} = {neg_difference:.3f}[/]"
            )
            self.console.print(
                f"  [dim]• Sum_neg = max(0, {old_cumsum_neg:.3f} + {neg_difference:.3f}) = {self.cumulative_sum_neg:.3f}[/]"
            )
            self.console.print(
                f"  [dim]• Adaptive threshold: {self.adaptive_threshold:.3f}[/]"
            )
            self.console.print(
                f"  [dim]• Upward change test: {self.cumulative_sum_pos:.3f} > {self.adaptive_threshold:.3f} = {'UPWARD CHANGE!' if self.cumulative_sum_pos > self.adaptive_threshold else 'No change'}[/]"
            )
            self.console.print(
                f"  [dim]• Downward change test: {self.cumulative_sum_neg:.3f} > {self.adaptive_threshold:.3f} = {'DOWNWARD CHANGE!' if self.cumulative_sum_neg > self.adaptive_threshold else 'No change'}[/]"
            )
        ...
        if change_detected:
            ...
            self.console.print(
                f"[bold magenta][STPH] CHANGE DETECTED! "
                f"{self.reference_mean:.3f}Hz → {freq:.3f}Hz "
                f"({percent_change:.1f}% {change_type})[/]"
            )
            self.console.print(
                f"[magenta][STPH] Sum_pos={self.cumulative_sum_pos:.2f}, Sum_neg={self.cumulative_sum_neg:.2f}, "
                f"Adaptive_Threshold={self.adaptive_threshold:.3f} (σ={self.rolling_std:.3f})[/]"
            )
            self.console.print(
                f"[dim magenta]STPH ANALYSIS: Cumulative sum exceeded adaptive threshold {self.adaptive_threshold:.2f}[/]"
            )
            self.console.print(
                f"[dim magenta]Detection method: {'Positive sum (upward trend)' if upward_change else 'Negative sum (downward trend)'}[/]"
            )
            self.console.print(
                f"[dim magenta]Adaptive minimum detectable change: {self.adaptive_delta:.3f}[/]"
            )

            if self.shared_resources and hasattr(
                self.shared_resources, "detector_change_count"
            ):
                if hasattr(self.shared_resources, "detector_lock"):
                    with self.shared_resources.detector_lock:
                        self.shared_resources.detector_change_count.value += 1
