...
def log_to_gui_and_console(
    console: Console, message: str, log_type: str = "info", data: dict = None
):
...
def get_change_detector(shared_resources: SharedResources, algorithm: str = "adwin"):
    ...
    if algo == "cusum":
        detector = CUSUMDetector(
            ...
            show_init=show_init_message,
            verbose=True,
        )
    elif algo == "ph":
        detector = SelfTuningPageHinkleyDetector(
            shared_resources=shared_resources, show_init=show_init_message, verbose=True
        )
    else:
...
def window_adaptation(
    ...
    if np.isnan(freq):
        ...
        if detector_samples > 0:
            last_freq = (
                shared_resources.detector_frequencies[-1]
                if shared_resources.detector_frequencies
                else "None"
            )
    ...
    if change_detected and change_log:
        ...
        if safe_adaptive_start >= 0 and (t_e - safe_adaptive_start) >= min_window_size:
            ...
            algorithm_name = (
                args.online_adaptation.upper()
                if hasattr(args, "online_adaptation")
                else "UNKNOWN"
            )
    ...
    if not np.isnan(freq) and freq > 0:
        ...
        if "frequency_hits" in args.window_adaptation and not change_detected:
            ...
            "data" in args.window_adaptation
            and len(shared_resources.data) > 0
    ...
    if not np.isnan(freq):
        ...
        recent_freqs = (
            list(shared_resources.detector_frequencies)[-5:]
            if len(shared_resources.detector_frequencies) >= 5
            else list(shared_resources.detector_frequencies)
        )
        ...
        if samples > 1:
            text += (
                f"[cyan]Recent freq history: {[f'{f:.3f}Hz' for f in recent_freqs]}[/]\n"
            )
