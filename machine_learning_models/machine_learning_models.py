...
class BandwidthDataset(Dataset):
    ...
    def __init__(self, data_list, num_parts=3):
        ...
        Args:
            data_list: Lists containing multiple partial sequences of list[ranks, bandwidth, duration, start_time]
        """
        self.data = []
        ...
        for seq in data_list:
            ...
            parts = [bandwidth[i*part_len:(i+1)*part_len] for i in range(self.num_parts-1)]
...
class HybridModel(nn.Module):
    ...
    def __init__(self, emb_dim=128, n_heads=4, ff_dim=128, num_layers=6, lstm_hidden=128):
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=emb_dim, nhead=n_heads,
...
def train_hybrid_model(file_or_directory, bandwidth_cutoff = -1, load_state_dict_and_optimizer_state = None,
               emb_dim = 128, n_heads = 4, ff_dim = 128, num_layers=3, epochs= 10, lr = 1e-3, save = False,
    ...
    if os.path.isfile(file_or_directory):
        ...
        files = [os.path.join(file_or_directory, f) for f in os.listdir(file_or_directory) if os.path.isfile(os.path.join(file_or_directory, f))]

    if not files:
        raise ValueError("No file(s) found.")

    # the initialised model
    model = HybridModel(emb_dim=emb_dim, n_heads=n_heads, ff_dim=ff_dim, num_layers=num_layers)
    load_optimizer = None
    ...
    for x in files:
        ...
        if x.endswith(".darshan") | x.endswith(".json"):
            if additional_ftio_args is not None:
                frequency, set = extract(cmd_input=["ftio", x, additional_ftio_args], bandwidth_cutoff=bandwidth_cutoff)
            else:
                frequency, set = extract(cmd_input=["ftio", x], bandwidth_cutoff=bandwidth_cutoff)
        ...
        # Calculate z-score and throw out traces with possible outliers, which might negatively influence the data
        # not used anymore, but may hold additional context / reasoning / on development
        #mean = np.mean(set)
        #deviation = np.std(set)
        #if deviation == 0:
        #    deviation = 1
        #z_scores = [(x - mean) / deviation for x in set]
        #outliers = False
        #for o in z_scores:
        #    if (o[0] > 3) | (o[0] < -3):
        #        outliers = True
        ...
        load_optimizer = train_model(model, dataset, epochs=epochs, lr=lr, load_optimizer=load_optimizer)
...
def train_model(model, dataset, epochs = 3, lr= 1e-3, load_optimizer = None, optimizer = None):
    ...
    for epoch in range(epochs):
        ...
        for past, future in dataset:
            ...
            loss = (0.3*F.huber_loss(pred_future, future)) + (0.7*spectral_loss(pred_future, future))
            loss.backward()
...
def predict_next_sequence(model_or_pthfile, file_or_directory, bandwidth_cutoff = 0, additional_ftio_args = None, only_data = None):
...
def spectral_loss(pred, target):
    """ Basic spectral loss funcion based on one-dimensional fourier transform
    Args:
...
def train_arima(file_or_directory, max_depth = 3, model_architecture = "SARIMA"):
    ...
    for sequence in sequences:
        ...
        for value in sequence[1]:
            value[0] = (value[0] - min_val_initial) / (max_val_initial - min_val_initial + 1e-8)
        ...
        for y in range(1):
            partial_sequence = pd.Series([x[y] for x in trainings_part], pd.Index(np.arange(len([x[y] for x in trainings_part]), dtype="int64" )))
            ...
            for q in range(8):
                try:
                    if model_architecture == "SARIMA":
                        model = SARIMAX(partial_sequence, order = (p,d,q), seasonal_order=(0, 0, 0, len(future_part) ))
                    else:
